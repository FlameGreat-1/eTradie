...
# AUTO_LOGIN_PHASE5_MARKETWATCH_SETTLE_SECS: how long to wait after
#   the Phase 3 submit before opening the New Chart dialog. MT5
#   dispatches the broker's symbol-catalogue request immediately
#   after a successful login; on a fresh prefix the first batch lands
#   within 1-2s. 3s is the safe ceiling under fluxbox + Xvfb.
# AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_WAIT_SECS: per-attempt budget
#   for the symbol-picker dialog to appear after Ctrl+N. The dialog
#   is rendered by MT5's Win32 dialog template, not the broker, so it
#   appears in ~200ms on the happy path; 15s covers slow-I/O cases.
# AUTO_LOGIN_PHASE5_CHART_WINDOW_WAIT_SECS: budget for the chart
#   window itself to appear after the symbol-picker is dismissed.
#   MT5 creates the chart synchronously before the dialog closes, so
#   the chart window is normally visible within ~500ms.
# AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_TITLE_REGEX: WM_NAME pattern
#   for the symbol-picker dialog. MT5 build 5836 uses 'Specification'
#   as the title on the new-chart dialog; older builds used
#   'New Chart' / 'Symbols'. The regex covers all three.
# AUTO_LOGIN_PHASE5_CHART_WINDOW_TITLE_REGEX: WM_NAME pattern for an
#   open MT5 chart window. The MT5 chart title format is
#   '<SYMBOL>,<TIMEFRAME>' (e.g. 'EURUSD,H1'). Symbol characters
#   include broker-specific suffix punctuation: '.', '#', '_', '-',
#   plus alphanumerics; the regex is intentionally permissive so a
#   broker with EURUSD.m / EURUSD-cd / XAUUSD# all match.
...
#   EA's typical OnInit -> bind latency (~1s); 30s gives headroom
#   without burning much of the total budget.
AUTO_LOGIN_PHASE5_ENABLED="${AUTO_LOGIN_PHASE5_ENABLED:-1}"
AUTO_LOGIN_PHASE5_MARKETWATCH_SETTLE_SECS="${AUTO_LOGIN_PHASE5_MARKETWATCH_SETTLE_SECS:-3}"
AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_WAIT_SECS="${AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_WAIT_SECS:-15}"
AUTO_LOGIN_PHASE5_CHART_WINDOW_WAIT_SECS="${AUTO_LOGIN_PHASE5_CHART_WINDOW_WAIT_SECS:-10}"
AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_TITLE_REGEX="${AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_TITLE_REGEX:-^(Specification|New Chart|Symbols)}"
...
# _drv_find_new_chart_dialog: search Xvfb for the symbol-picker
# dialog raised by Ctrl+N. Returns the WID or empty.
_drv_find_new_chart_dialog() {
  _drv_find_window_by_regex "$AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_TITLE_REGEX"
}

# _drv_find_chart_window: search Xvfb for an open MT5 chart window
# (title is '<SYMBOL>,<TIMEFRAME>'). Returns the WID or empty.
_drv_find_chart_window() {
  _drv_find_window_by_regex "$AUTO_LOGIN_PHASE5_CHART_WINDOW_TITLE_REGEX"
}

# _drv_wait_for_new_chart_dialog: poll _drv_find_new_chart_dialog
# until visible or budget exhausted. Echoes WID on success, returns 0;
# returns 1 on timeout. Same shape as _drv_wait_for_dialog so Phase 5
# is symmetric with Phase 2c.
_drv_wait_for_new_chart_dialog() {
  local _budget="$1"
  local _waited=0 _wid
  while [ "$_waited" -lt "$_budget" ]; do
    _wid=$(_drv_find_new_chart_dialog)
    if [ -n "$_wid" ]; then
      printf '%s' "$_wid"
      return 0
    fi
...
# _drv_phase5_chart_attach: open a chart on the broker-default
# Market Watch symbol so MT5's startup.ini-pinned 'Template=expert'
# directive auto-applies the EA template, the EA's OnInit runs, and
# the EA binds tcp://*:5555.
#
# Why this exists
# ---------------
...
# LISTENing past the 240s auto-login budget. Without a chart there
# is no template surface, no EA OnInit, no socket bind, the watchdog
# stays unhealthy, and the engine's readiness gate eventually
# tears the StatefulSet down.
#
# How it works
# ------------
# 1. Wait AUTO_LOGIN_PHASE5_MARKETWATCH_SETTLE_SECS so MT5's
#    post-login broker symbol-catalogue request lands and Market
#    Watch is populated (otherwise the New Chart dialog's symbol
#    list is empty).
...
# 3. Clear any blocking modals (Welcome to LiveUpdate, etc.) so
#    the main window holds keyboard focus.
# 4. Send Ctrl+N (canonical MT5 'New Chart Window' hotkey, stable
#    since build 1000+). MT5 raises the symbol-picker dialog
#    (build 5836 title: 'Specification'; older builds: 'New Chart'
#    or 'Symbols' -- regex covers all three).
# 5. On dialog visible: send Return to accept the default-selected
#    symbol (Win32 listbox default is index 0, which is whatever
#    the broker put first in Market Watch). The dialog closes and
#    MT5 opens a chart.
# 6. Wait for a chart window WID matching '<SYMBOL>,<TIMEFRAME>'.
# 7. MT5 auto-applies startup.ini [Charts] Template=expert to the
#    newly-opened chart -- per MetaQuotes the directive applies to
#    charts opened both at startup AND interactively post-startup.
#    The pre-staged expert.tpl loads ZeroMQ_EA, OnInit runs,
#    g_socket.bind("tcp://*:5555") fires.
# 8. Poll :5555 LISTEN for AUTO_LOGIN_PHASE5_BIND_WAIT_SECS. On
#    bind we exit success; on timeout we fall through to Phase 4
#    which keeps polling the remaining budget.
#
# Contract
# --------
#   * $1 = main UI window WID (used only for focus clearing).
#   * Best-effort: every failure logs WARN and returns 1; the
#     caller (auto_login_driver) MUST treat a non-zero return as
#     'try harder in Phase 4', never as fatal.
#   * Idempotent: re-running on an already-:5555-LISTEN pod is a
#     no-op (early return at step 2).
#   * Never logs credentials. Logs only WIDs, names, and stage markers.
_drv_phase5_chart_attach() {
  local _mwid="$1"
  local _dialog_wid _chart_wid _waited=0
  ...
  _drv_log "phase5: settling ${AUTO_LOGIN_PHASE5_MARKETWATCH_SETTLE_SECS}s for Market Watch population"
  sleep "$AUTO_LOGIN_PHASE5_MARKETWATCH_SETTLE_SECS"

  # Step 2: subsequent-boot short-circuit.
  if _drv_zmq_bound; then
    _drv_log "phase5: :5555 already LISTEN (accounts.dat fast path); skipping chart-attach"
    return 0
  fi

  # Step 3: make sure no modal owns focus before we keystroke.
  if [ -n "$_mwid" ]; then
  ...
  # Step 4: open the symbol-picker dialog.
  _drv_log "phase5: sending Ctrl+N to open New Chart dialog"
  DISPLAY=:99 _xdo key --clearmodifiers ctrl+n 2>/dev/null || true

  _dialog_wid=$(_drv_wait_for_new_chart_dialog "$AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_WAIT_SECS")
  if [ -z "$_dialog_wid" ]; then
    _drv_warn "phase5: New Chart dialog did not appear within ${AUTO_LOGIN_PHASE5_NEW_CHART_DIALOG_WAIT_SECS}s after Ctrl+N"
    return 1
  fi
  _drv_log "phase5: New Chart dialog WID=${_dialog_wid} visible; accepting default symbol"

  # Step 5: accept default-selected symbol.
  DISPLAY=:99 _xdo windowactivate "$_dialog_wid" 2>/dev/null || true
  sleep 0.3
  DISPLAY=:99 _xdo key --clearmodifiers Return 2>/dev/null || true

  # Step 6: wait for an open chart window.
  _chart_wid=$(_drv_wait_for_chart_window "$AUTO_LOGIN_PHASE5_CHART_WINDOW_WAIT_SECS")
  if [ -z "$_chart_wid" ]; then
    _drv_warn "phase5: chart window did not appear within ${AUTO_LOGIN_PHASE5_CHART_WINDOW_WAIT_SECS}s after Return; MT5 may have rejected the default symbol"
    return 1
  fi
  _drv_log "phase5: chart window WID=${_chart_wid} visible; expert.tpl auto-applies via startup.ini"

  # Step 8: poll :5555 LISTEN.
  while [ "$_waited" -lt "$AUTO_LOGIN_PHASE5_BIND_WAIT_SECS" ]; do
    if _drv_zmq_bound; then
      _drv_log "phase5: :5555 LISTEN at +${_waited}s after chart open; EA OnInit succeeded"
      return 0
    fi
    sleep 1
    _waited=$(( _waited + 1 ))
  done
  _drv_warn "phase5: :5555 not LISTEN within ${AUTO_LOGIN_PHASE5_BIND_WAIT_SECS}s after chart open; falling through to Phase 4 poll"
  return 1
}
...
auto_login_driver() {
  ...
  if [ "$dialog_seen" -eq 1 ]; then
    ...
    # Phase 5 (chart-attach). On build 5836 + /portable + fresh
    # Wine prefix, MT5 does NOT open a chart on its own after login;
    # without a chart the startup.ini-pinned 'Template=expert' has
    # nowhere to apply, the EA never loads, and :5555 never binds.
    # Phase 5 keystrokes Ctrl+N (canonical MT5 'New Chart Window'
    # hotkey) so MT5 opens a chart on the broker-default Market
    # Watch symbol; MT5 then auto-applies expert.tpl per
    # startup.ini's Charts.Template directive (which applies to
    # charts opened both at startup AND interactively post-launch),
    # the EA OnInit runs, and the EA binds :5555.
    #
    # Best-effort: any Phase 5 failure logs WARN and falls through
    # to Phase 4 (poll :5555 for the remaining budget). A broker
    # that happens to auto-open a chart on login is still serviced
    # by the Phase 4 poll.
    #
    ...
    if _drv_phase5_chart_attach "$main_wid"; then
      _drv_log "phase5: chart-attach succeeded; :5555 bound"
      return 0
    else
