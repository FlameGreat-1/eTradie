...
def upgrade() -> None:
    ...
    if table_name not in inspector.get_table_names():
        ...
        if 'temperature' not in existing_columns:
            op.add_column(table_name, sa.Column("temperature", sa.Float(), nullable=False, server_default="0.0"))
        
        if 'max_output_tokens' not in existing_columns:
            op.add_column(table_name, sa.Column("max_output_tokens", sa.Integer(), nullable=False, server_default="16384"))
        
        if 'is_active' not in existing_columns:
            op.add_column(table_name, sa.Column("is_active", sa.Boolean(), nullable=False, server_default="false"))
        
        if 'label' not in existing_columns:
            op.add_column(table_name, sa.Column("label", sa.String(100), nullable=False, server_default=""))
