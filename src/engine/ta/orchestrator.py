...
class TAOrchestrator:
        ...
        try:
            ...
            for i in range(len(available_htfs) - 1):
                ...
                if pulse:
                    await pulse.emit(
                        "SHIMMING",
                        f"Scanning {higher_tf.value}→{lower_tf.value} SMC zones",
                    )
                ...
                if pulse:
                    await pulse.emit(
                        "SHIMMING",
                        f"Scanning {higher_tf.value}→{lower_tf.value} SnD zones",
                    )
            ...
            for i in range(len(available_ltfs) - 1):
                ...
                if pulse:
                    await pulse.emit(
                        "PONTIFICATING",
                        f"LTF confirmation {higher_tf.value}→{lower_tf.value}",
                    )
                smc = self._run_smc_detection(
                    sequences[higher_tf],
                    sequences[lower_tf],
    ...
    def _build_result(
        ...
        *,
        ...
        return {
            ...
            "smc_candidates_count": len(deduped_smc),
            "snd_candidates_count": len(deduped_snd),
    ...
    def _build_snapshot_metadata(snapshot: TechnicalSnapshot) -> dict[str, Any]:
        ...
        live_breakers = sum(1 for bb in snapshot.breaker_blocks if not bb.mitigated)
    ...
    def _serialize_snapshot(self, snapshot: TechnicalSnapshot) -> dict[str, Any]:
        ...
        live_breakers = [bb for bb in snapshot.breaker_blocks if not bb.mitigated]
        ...
        out: dict[str, Any] = {
            ...
            "supply_zones": self._serialize_supply_zones(live_supply[-arr_cap:]),
            "demand_zones": self._serialize_demand_zones(live_demand[-arr_cap:]),
        }
