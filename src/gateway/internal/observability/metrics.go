...
// GatewayRateLimitedTotal counts requests rejected with HTTP 429 by a
// per-user token-bucket limiter. The `route` label is the limiter
// scope ("cycle_run" for the dedicated tiered limiter on
// POST /api/v1/cycle/run, "api_default" for the shared catch-all
// limiter on the other mutating /api/v1/* routes); `tier` is the
