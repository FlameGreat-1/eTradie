...
func (b *Bridge) placeOrder(ctx context.Context, order *models.OrderPlacement, orderType string) (*models.OrderResult, error) {
	...
	// Normalise partial fill: some bridge versions emit Status="FILLED"
	// but VolumeFilled < requested LotSize. Flip to PARTIALLY_FILLED
	// so downstream knows to act on the remainder.
	status := resp.Status
	if status == "FILLED" && resp.VolumeFilled > 0 && resp.VolumeFilled < order.LotSize {
		status = "PARTIALLY_FILLED"
	}

	volumeFilled := resp.VolumeFilled
	volumeRemaining := resp.VolumeRemaining
	if volumeFilled == 0 && status == "FILLED" {
		volumeFilled = order.LotSize
	}
	if volumeRemaining == 0 && volumeFilled > 0 && volumeFilled < order.LotSize {
		volumeRemaining = order.LotSize - volumeFilled
	}
