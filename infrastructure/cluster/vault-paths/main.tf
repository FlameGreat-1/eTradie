...
resource "vault_kv_secret_v2" "engine" {
  ...
  delete_all_versions = false
  data_json = jsonencode({
    bootstrap = "placeholder; populate keys: database_url, postgres_user, postgres_password, redis_url, redis_password, broker_encryption_key, auth_jwt_secret, cftc_app_token, fred_api_key, twelvedata_api_key, processor_anthropic_api_key, processor_openai_api_key, processor_gemini_api_key, mt5_metaapi_token, engine_internal_shared_secret. broker_encryption_key is the ONLY credential KEK in the platform (the engine is the sole consumer; gateway/execution/management do NOT hold it) and is KEK version 1 for credential-at-rest envelope encryption. To ROTATE the credential KEK, add broker_encryption_key_v<n> (n>=2, e.g. broker_encryption_key_v2 = openssl rand -hex 32) and declare it in helm/engine values externalSecrets.engine.rotationKeyVersions; the engine activates the highest version, the re-wrap routine migrates rows, then remove the old version to revoke it. engine_internal_shared_secret is the X-Internal-Auth shared secret the engine VALIDATES on every /internal/* call (src/engine/shared/internal_auth.py); the gateway, execution, and management services SEND this header on every internal call, so this value MUST byte-equal etradie/services/gateway/${var.environment}:engine_internal_shared_secret AND etradie/services/execution/${var.environment}:engine_internal_shared_secret AND etradie/services/management/${var.environment}:engine_internal_shared_secret. Generate once: openssl rand -hex 32, then populate all four Vault paths with the same value. Note: rag_chroma_auth_token is NOT in this path; populate etradie/data-layer/chromadb/${var.environment}:auth_token instead (single source of truth shared with the ChromaDB server)."
  })
  lifecycle {
    ignore_changes = [data_json]
  }
...
resource "vault_kv_secret_v2" "execution" {
  ...
  delete_all_versions = false
  data_json = jsonencode({
    bootstrap = "placeholder; populate keys: auth_database_url, auth_admin_password, execution_database_url, execution_redis_url, auth_jwt_secret, engine_internal_shared_secret. auth_database_url MUST equal etradie/services/gateway/${var.environment}:auth_database_url -- the shared src/auth package (used by every Go service) reads it via envconfig prefix AUTH and validates required in production/staging; the auth tables are SHARED across gateway/execution/management so all three services point at the same DSN. auth_admin_password MUST equal etradie/services/gateway/${var.environment}:auth_admin_password -- same shared auth package, same shared admin user; the pod fails fast at startup without it (HasAdminSeedPassword() check). auth_jwt_secret MUST equal etradie/services/gateway/${var.environment}:auth_jwt_secret. engine_internal_shared_secret MUST equal etradie/services/gateway/${var.environment}:engine_internal_shared_secret AND the engine's value -- execution sends it in X-Internal-Auth on every /internal/broker/* call and the pod fails fast at startup without it UNCONDITIONALLY in production/staging (the mt5 bridge is the sole broker.Port implementation in the execution service, so every call needs it). NOTE: no broker/credential encryption KEK here -- execution does not encrypt credentials; it reaches the broker via the engine /internal/broker/* bridge."
  })
  lifecycle {
    ignore_changes = [data_json]
  }
...
resource "vault_kv_secret_v2" "management" {
  ...
  delete_all_versions = false
  data_json = jsonencode({
    bootstrap = "placeholder; populate keys: auth_database_url, auth_admin_password, management_database_url, management_redis_url, auth_jwt_secret, engine_internal_shared_secret. auth_database_url MUST equal etradie/services/gateway/${var.environment}:auth_database_url -- the shared src/auth package (used by every Go service) reads it via envconfig prefix AUTH and validates required in production/staging; the auth tables are SHARED across gateway/execution/management so all three services point at the same DSN. auth_admin_password MUST equal etradie/services/gateway/${var.environment}:auth_admin_password -- same shared auth package, same shared admin user; the pod fails fast at startup without it (HasAdminSeedPassword() check). auth_jwt_secret MUST equal etradie/services/gateway/${var.environment}:auth_jwt_secret. engine_internal_shared_secret MUST equal etradie/services/gateway/${var.environment}:engine_internal_shared_secret AND the engine's value -- management sends it in X-Internal-Auth on every /internal/broker/* call and the pod fails fast at startup without it UNCONDITIONALLY in production/staging (the mt5 bridge is the sole broker.Port implementation in the management service, so every call needs it). NOTE: no broker/credential encryption KEK here -- management does not encrypt credentials; it reaches the broker via the engine /internal/broker/* bridge."
  })
  lifecycle {
    ignore_changes = [data_json]
  }
