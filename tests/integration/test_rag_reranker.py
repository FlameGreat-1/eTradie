...
class TestDocTypeBoost:
    def test_rulebook_outranks_lower_weighted(self, reranker):
        ...
        ranked = reranker.rerank(
            [_chunk(DocumentType.CHART_SCENARIO_LIBRARY, 0.70), _chunk(DocumentType.MASTER_RULEBOOK, 0.60)]
        )
    ...
    def test_smc_boosted(self, reranker):
        ...
        ranked = reranker.rerank(
            [_chunk(DocumentType.CHART_SCENARIO_LIBRARY, 0.65), _chunk(DocumentType.SMC_FRAMEWORK, 0.62)]
        )
...
class TestSectionBonus:
    def test_bonus(self, reranker):
        ...
        ranked = reranker.rerank(
            [
                _chunk(DocumentType.CHART_SCENARIO_LIBRARY, 0.80),
                _chunk(DocumentType.CHART_SCENARIO_LIBRARY, 0.80, "rules", "entry"),
            ]
        )
