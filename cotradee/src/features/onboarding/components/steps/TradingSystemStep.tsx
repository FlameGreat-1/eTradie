  ...
  if (isDone) {
    return (
      <div className="flex flex-col items-center gap-6 text-center">
        ...
        <button onClick={onComplete} className="inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3 text-sm font-semibold text-black hover:bg-white/90">Continue</button>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="w-full max-w-md mx-auto text-center">
        <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-white/5 border border-white/10"><Cpu className="h-6 w-6 text-content" /></div>
        <h2 className="text-xl font-bold text-content">Build your trading system</h2>
        <button onClick={() => setBuilderOpen(true)} className="mt-8 w-full rounded-xl bg-white p-3 font-bold text-black">Start the builder</button>
      </div>
