...
## 12. COMPLETE DASHBOARD ENDPOINT MAP SUMMARY

### Authentication (`/auth/*` on Gateway)
| Method | Path | Action | Role Support |
|---|---|---|---|
| POST | `/auth/login` | Log in | Public |
| POST | `/auth/register` | Sign up | Public |
| POST | `/auth/refresh` | Renew token | Public |
| POST | `/auth/logout` | Discard active token | Authenticated |
| POST | `/auth/logout-all` | Purge all devices | Authenticated |
| GET | `/auth/me` | User Profile details | Authenticated |
| PUT | `/auth/me/password` | Account security update | Authenticated |

### Admin Area (Gateway / Engine)
| Method | Path | Action | Role Support |
|---|---|---|---|
| GET | `/auth/admin/users` | List Users | Admin |
| POST | `/auth/admin/users` | Manual user create | Admin |
| PUT | `/auth/admin/users/{id}/activate` | Unsuspend user | Admin |
| PUT | `/auth/admin/users/{id}/deactivate` | Revoke user login | Admin |
| GET | `/api/processor/models` | Global Provider Listing | Admin |
| GET | `/api/processor/config` | Global Base LLM Setting | Admin |
| PUT | `/api/processor/config` | Overwrite Base LLM | Admin |

### WebSockets & Notifications (Gateway)
| Method | Path | Action | Role Support |
|---|---|---|---|
| WS | `/ws/notifications` | Subscribe to realtime events stream | Authenticated |
| GET | `/events/recent` | Initial load on dashboard launch | Authenticated |
| GET | `/events/since` | Gap-fill historical events | Authenticated |

### AI Analysis Pipeline (Gateway / Engine)
| Method | Path | Action | Role Support |
|---|---|---|---|
| GET | `/api/v1/config` | See cycle limits | Authenticated |
| PUT | `/api/v1/config/interval` | Adjust timer frequencies | Authenticated |
| GET | `/api/v1/symbols` | Selected symbols | Authenticated |
| PUT | `/api/v1/symbols` | Change pair watchlists | Authenticated |
| POST | `/api/v1/symbols/reset` | Emergency pair reset | Authenticated |
| POST | `/api/v1/cycle/run` | Manual Full Pipeline fire | Authenticated |
| GET | `/api/analysis/latest` | Feed layout | Authenticated |
| GET | `/api/analysis/history` | Historical archive table | Authenticated |
| GET | `/api/analysis/stats` | Analytics widgets | Authenticated |
| GET | `/api/analysis/{id}` | Trade detail modal | Authenticated |
| POST | `/api/analysis/rerun` | Re-request AI output explicitly | Authenticated |

### Individual Connections (Engine)
| Method | Path | Action | Role Support |
|---|---|---|---|
| GET | `/api/llm/providers` | Allowed external AI engines list | Authenticated |
| GET | `/api/llm/connections` | Check User's Keys list | Authenticated |
| GET | `/api/llm/connections/active`| Return in-use API key profile | Authenticated |
| ... | *`(CRUD routes on LLM & Broker endpoints omitted for brevity, identical implementations)`* | Full Config Support | Authenticated |

### Trade Execution & Live Data (Execution / Management)
| Method | Path | Action | Role Support |
|---|---|---|---|
| GET | `/api/v1/settings` | See Risk Limits | Authenticated |
| PUT | `/api/v1/settings` | Update Risk Limits | Authenticated |
| GET | `/api/v1/account` | Current Balance Sheet | Authenticated |
| GET | `/api/v1/state` | PnL/Live trades mapping | Authenticated |
| POST | `/api/v1/orders/cancel` | Panic clear limit order | Authenticated |
| GET | `/api/v1/management/trades` | See TP/SL active changes | Authenticated |
| GET | `/api/v1/management/journal` | Ledger table view | Authenticated |
| GET | `/api/v1/management/metrics` | Dashboard chart data | Authenticated |
